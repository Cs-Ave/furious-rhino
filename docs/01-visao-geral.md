...
- **Medalhas** — 27 conquistas, de "Primeira Fuga" a "Lenda do Mundo" (vencer o chefe do fim do mundo).
...
- **Estúdio do criador** — uma página especial (`/?setup`, protegida por chave, aberta com um duplo-clique num atalho) com quatro abas: **🎨 Skins** (transformar folhas de desenhos geradas por IA em skins prontas e decidir como cada uma é conquistada — grátis, pódio, façanha ou totais de vida), **🖼️ Sprites** (o catálogo de todos os personagens do jogo, animado, com os parâmetros de cada um editáveis — velocidade, pulo, caixa de colisão — e um gerador que cria inimigos novos, até espécies inéditas, sem programar), **📊 Radiografia** (a análise completa de como os jogadores usam o jogo, com diagnósticos automáticos) e **🆘 Recuperação** (devolver a identidade a quem reinstalou o app e perdeu o apelido — o jogador pede pelo botão 🆘, o criador confere e autoriza, e o jogo do jogador se restaura sozinho). Tudo com uma rede de segurança: cada gravação roda a bateria de testes e se desfaz sozinha se algo quebrar; nada vai ao ar sem a publicação seguinte.
