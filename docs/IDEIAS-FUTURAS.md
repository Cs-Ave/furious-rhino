...
| **v1.12.4 "O Desafio"** ✅ 06/09 | a ideia G: desafio por link (`?desafio=<m>&de=<nome>`, `LinkChallenge`), estaca do amigo, DEVOLVER, letra `md`; "novidades desde a última visita" (`CHANGELOG_CARDS`); "alguém passou você"; modo re-jogo; sobras da Régua (R-06/R-07 na janela, Barreira/Faraó impressos, `#medals-modal` removido) |

### Aprovado, ainda não executado

- **v1.12.4 "O Desafio"** ✅ 06/09 (nos portões; era a v1.12.3 antes de o pacote P virar release própria) — a ideia **G** (§766) saiu do banco. Executada como descrito abaixo, com duas diferenças de projeto: o 📤 do fim de corrida MORFA em "DEVOLVER O DESAFIO" (um botão só, o rodapé fixo da Régua não cresce) e TODO share passa a carregar o link (é o lado remetente — sem ele o link nunca circularia). A carona de skins não entrou (sem arte do dono). Spec original: `/?desafio=<m>&de=<nome>` com sanitização estrita (clamp 1..10000, `de` 3-12 chars whitelist, `textContent`, banner nunca é link), estaca do amigo na pista via `createTrackMarks`, "devolver o desafio" no fim de corrida (sem apelido abre o `#nickname-modal` — é onde o visitante vira jogador), persistência local 7 dias, letra `md:1`. Mais: cards **"novidades desde a sua última visita"** (tabela `CHANGELOG_CARDS` em código + `NewsSystem.push`, zero rede) e **"alguém passou você"** (estende o `podium:in/out` para qualquer rank cacheado). Carona: skins alcançáveis (`meters 300/600`, `escaped`, `streakBest 3`) se o dono fizer a arte.
