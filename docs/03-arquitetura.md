...
- **O pacote do CASO 1 (v1.10.1)**: `PHYSICS_MAX_DELTA_MS` clampa o delta do `world.update` (wrap, não fork — quadro de segundos vira UM passo de câmera lenta, nunca teleporte), e o retrato PAUSA de verdade (listener de `matchMedia` + o caso da largada em pé, que precisa entrar DENTRO da graça de 150 ms do `started`). O relógio do loop (`i`) segue no delta real — a sonda que separa congelamento de salto continua honesta.
