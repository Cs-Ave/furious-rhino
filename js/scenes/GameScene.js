...
export class GameScene extends Phaser.Scene {
  ...
  setupStartScreen() {
    ...
    // v1.9.0 — Recuperação de identidade (o caso "Teco"): só age se houver
    ...
    // v1.9.3: o toque já foi armado pelo HomeScreen no boot, antes de esta
    // cena existir — aqui a cena só se apresenta como quem sabe começar a
    // corrida. Se o jogador tocou enquanto o motor carregava, o `ready`
    // dispara a partida AGORA, sem exigir um segundo toque. (As guardas de
    // P/ESC e de modal aberto vivem no armStart; o `startTriggered` do
    // startRun impede partida dupla.)
    HomeScreen.ready(() => this.startRun());
  ...
  setupLeaderboardUI() {
    ...
    document.getElementById('ranking-close').addEventListener('click', () => {
      ...
      // v1.9.3: o modal do top 10 acabou de reescrever os caches do pódio e
      // do rank (fetchTop10 → setPodium + setLastRank). Sem esta repintura a
      // home atrás do modal continuava com o desenho anterior.
      HomeScreen.repaint();
    });
    ...
    // v1.9.0: "era meu" — pedido de recuperação mediado pelo administrador
  ...
  // v1.9.3: a pintura da home mudou-se para js/home/HomeScreen.js —
  // estes wrappers existem porque a cena chama estes nomes em dezenas de
  // pontos. Quem pinta é um módulo só, e ele não depende do Phaser.
  showRank(rank) { HomeScreen.showRank(rank); }

  topCauseLabel() { return HomeScreen.topCauseLabel(); }

  renderStartChart() { HomeScreen.renderStartChart(); }

  renderPodium() { HomeScreen.renderPodium(); }

  renderYouStep() { HomeScreen.renderYouStep(); }

  updatePodiumGap(rank) { HomeScreen.updatePodiumGap(rank); }
  ...
  onRankRefreshed(rank) {
    // v1.9.3: era só `showRank(rank)` — o número mudava e o pódio, o degrau
    // VOCÊ e a provocação continuavam com o dado do boot anterior. Repinta
    // tudo de uma fonte só.
    HomeScreen.repaint();
  ...
  renderChallenges() { HomeScreen.renderChallenges(); }

  paintChallenges(box) { HomeScreen.paintChallenges(box); }

  buildChallengeCard(ch, myId, ended) { return HomeScreen.buildChallengeCard(ch, myId, ended); }

  paintChallengeRows(card, rowsBox, ch, rows, myId, ended) {
    HomeScreen.paintChallengeRows(card, rowsBox, ch, rows, myId, ended);
  }

  showHomeToast(msg) { HomeScreen.showHomeToast(msg); }
  ...
  async saveNickname() {
    ...
    if (verdict === 'taken') {
      ...
      // v1.9.0: pode ser o PRÓPRIO doc do jogador (reinstalou o PWA → id
  ...
  // v1.9.0 — "🆘 Este apelido era meu": grava a marca local (que arma a
